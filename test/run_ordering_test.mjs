/**
 * Ordering logic test — runs the real `checkOrder` from ../ordering.js
 * against ground_truth_v3.json.
 *
 ...
 *
 * Pre-rewrite this imported the same functions by extracting them out of
 * index.html by string markers (see archive/test_items_backup/ for the old
 * version) — ordering.js is a straight, verified-identical extraction of
 * that same code (see scratchpad diff run during the rewrite), now a real
 * module instead of an eval'd string slice.
 *
...
// ── Convert ground_truth_v3 rows → book objects ───────────────────────────
...
console.log('ShelfCheck ordering test — checkOrder imported from ordering.js');
console.log('='.repeat(66));

for (const [imgName, data] of entries) {
  const section = data.section;
  const gtBooks = gtToBooks(data);
  const checked = checkOrder(gtBooks, section);
