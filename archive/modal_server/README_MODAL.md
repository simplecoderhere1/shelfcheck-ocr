...
> **Not the recommended deployment.** Modal requires a credit card, which this
> project can't use. Deploy `../hf_space` instead — a free Hugging Face Space,
> no card, CPU only.
>
> The GPU turned out to be unnecessary anyway. The phone was slow because it ran
> ONNX under **WASM**, not because the model is heavy: the same graph on two
> ordinary vCPUs does a shelf in **2.16s average** at identical recall (92.0%
> raw, measured on all 10 test images at `intra_op_num_threads=2`). This folder
> stays because `ocr_pipeline.py` and `validate_local.py` are shared with the
> Space, and in case a GPU host ever becomes available.
...
is $30 of compute every month, roughly 50 GPU-hours on the T4 this uses —
but it asks for a card on signup.
